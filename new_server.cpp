...
bool if_data_actually_read = false;
    ...
    while(1) {
		...
		for(int i = fd + 1; i <= fdmax && fda > 0; i++){
			if(FD_ISSET(i, &rmask)){
				...
				//read amount of data
				else if(get_state_of_client(i) == 1 && client_fd != i && !if_data_actually_read){
...
//				FD_CLR(i, &wmask);
//				if(i == fdmax)
//					while(fdmax > fd && ! FD_ISSET(fdmax, &mask))
//						fdmax -= 1;
//				printf("Closed\n");
			}
